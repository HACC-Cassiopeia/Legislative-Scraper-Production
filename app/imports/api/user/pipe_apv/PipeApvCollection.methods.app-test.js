...
if (Meteor.isClient) {
  describe('PIPEApvProfileCollection Meteor Methods', function testSuite() {
    it('Can define, update, and removeIt', async function test1() {
      ...
      const collectionName = PIPEApvProfiles.getCollectionName();
      ...
      expect(PIPEApvProfiles.isDefined(docID)).to.be.true;
      let doc = PIPEApvProfiles.findDoc(docID);
      ...
      doc = PIPEApvProfiles.findDoc(docID);
      ...
      expect(PIPEApvProfiles.isDefined(docID)).to.be.false;
    });
  });
}
